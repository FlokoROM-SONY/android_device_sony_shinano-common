...
#define BLUE_LED "/sys/class/leds/led:rgb_blue/brightness"
#define GREEN_LED "/sys/class/leds/led:rgb_green/brightness"
#define RED_LED "/sys/class/leds/led:rgb_red/brightness"
...
static void update_led(int charge_status) {
    ...
    if (!bled) {
        KLOG_ERROR(LOG_TAG, "%s: could not open blue LED: %s errno=%i (%s)\n",
            __func__, BLUE_LED, errno, strerror(errno));
        return;
    } else {
        ...
        if (!gled) {
            ...
            if (!rled) {
                fclose(gled);
                KLOG_ERROR(LOG_TAG, "%s: could not open red LED: %s errno=%i (%s)\n",
                    __func__, RED_LED, errno, strerror(errno));
                return;
        }
    }
